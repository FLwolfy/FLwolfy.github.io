var RENDERER = {
    ...
    bindEvent : function(){
        ...
        this.$container.onmousemove = this.moveEpicenter; // 注意：使用 onmousemove 而非 onmousemove 事件名称
    },
    ...
    render : function(){
        ...
        // 波浪偏移更新
        this.baseWaveOffset += 0.02;
        for(var i = 0; i < this.points.length; i++){
            this.points[i].initHeight = this.height * this.INIT_HEIGHT_RATE
                + Math.sin(this.points[i].x / this.width * Math.PI + this.baseWaveOffset) * 50; // 50 为振幅
        }
        ...
        const grey01 = getComputedStyle(root).getPropertyValue('--grey-1-a7').trim();
        this.context.fillStyle = grey01;
...
SURFACE_POINT.prototype = {
    ...
    init : function(){
        this.initHeight = this.renderer.height * this.renderer.INIT_HEIGHT_RATE
            + Math.sin(this.x / this.renderer.width * Math.PI + this.renderer.baseWaveOffset) * 50; // 初始波浪
